...
class EmbyService:
    ...
    def _cache_key(self, server_id: str | None = None) -> str:
        return server_id or "__default__"

    async def get_api_key(self, server_id: str | None = None) -> str:
    ...
    async def get_user_id(self, server_id: str | None = None) -> str:
        ...
        try:
            server_config = get_server_config(server_id)
            api_key = await self.get_api_key(server_id)
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    users = resp.json()
                    if users:
                        self._user_id_cache[cache_key] = users[0]["Id"]
                        return self._user_id_cache[cache_key]
        except Exception as e:
    ...
    async def get_item_info(self, item_id: str, server_id: str | None = None) -> dict:
        """获取媒体项目信息（包含海报等）"""
        cache_key = (self._cache_key(server_id), item_id)
        ...
        try:
            server_config = get_server_config(server_id)
            api_key = await self.get_api_key(server_id)
            user_id = await self.get_user_id(server_id)
            if not api_key or not user_id:
                return {}

            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{server_config['emby_url']}/emby/Users/{user_id}/Items/{item_id}",
                    params={
                        "api_key": api_key,
    ...
    async def get_poster(self, item_id: str, max_height: int = 300, max_width: int = 200, server_id: str | None = None) -> tuple[bytes, str]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{server_config['emby_url']}/emby/Items/{item_id}/Images/Primary",
                    params={
                        "api_key": api_key,
    ...
    async def get_backdrop(self, item_id: str, max_height: int = 720, max_width: int = 1280, server_id: str | None = None) -> tuple[bytes, str]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{server_config['emby_url']}/emby/Items/{item_id}/Images/Backdrop",
                    params={
                        "api_key": api_key,
                        "maxHeight": max_height,
                        "maxWidth": max_width,
    ...
    def get_poster_url(self, item_id: str, item_type: str, item_info: dict, server_id: str | None = None) -> str | None:
        """根据媒体信息获取海报 URL"""
        if not item_info:
            return None
        ...
        if item_type == "Episode" and item_info.get("SeriesId"):
            return f"/api/poster/{item_info['SeriesId']}{suffix}"
        elif item_info.get("ImageTags", {}).get("Primary"):
            return f"/api/poster/{item_id}{suffix}"
    ...
    def get_backdrop_url(self, item_id: str, item_type: str, item_info: dict, server_id: str | None = None) -> str | None:
        """根据媒体信息获取背景图(横版) URL"""
        if not item_info:
            return None
        suffix = f"?server_id={server_id}" if server_id else ""
        ...
        # 对于剧集，使用剧集背景图
        if item_type == "Episode" and item_info.get("SeriesId"):
            return f"/api/backdrop/{item_info['SeriesId']}{suffix}"
        # 检查是否有 Backdrop 图片
        elif item_info.get("BackdropImageTags") and len(item_info.get("BackdropImageTags", [])) > 0:
            return f"/api/backdrop/{item_id}{suffix}"

        return None

    async def get_item_images(self, item_id: str, server_id: str | None = None) -> dict:
        """获取项目的图片URL和概述信息"""
        try:
            item_info = await self.get_item_info(item_id, server_id)
            if not item_info:
                return {}
            ...
            return {
                "poster_url": self.get_poster_url(item_id, item_type, item_info, server_id),
                "backdrop_url": self.get_backdrop_url(item_id, item_type, item_info, server_id),
    ...
    async def get_now_playing(self, server_id: str | None = None) -> list[dict]:
        """获取当前正在播放的会话"""
        try:
            server_config = get_server_config(server_id)
            api_key = await self.get_api_key(server_id)
            ...
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{server_config['emby_url']}/emby/Sessions",
                    params={"api_key": api_key},
                    timeout=10
                )
                if resp.status_code == 200:
